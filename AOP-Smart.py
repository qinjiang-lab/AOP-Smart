...
def simplify_context(selected_ke, selected_ker, selected_aop, kers, key_events):
    global clean_text_ke, clean_text_ker, clean_text_ao
    ke_entries = []
...
# --------------------------
# Step 2: Construct context (FIXED: iterate with keys, not assuming 'id' field)
# --------------------------
def build_context_from_ke_ids(ke_ids):
    ...
    # Iterate over aops.items() to keep the key as the AOP ID
    ...
    # Iterate over kers.items() to keep the key as the KER ID
...
# --------------------------
def run_llm_thread():
    global llm_thread, llm_running, current_id
    if llm_running:
        messagebox.showinfo("Info", "LLM is already running. Please wait.")
        return
    current_id += 1
...
def run_llm(thread_id):
    global llm_running, thinking_running, last_result, current_id
    llm_running = True
    ...
    if not llm_running:
        ...
        return
    if not ke_ids:
        messagebox.showerror("Error", "No relevant KEs found.")
        reset_ui_state()
        return

    # Step 2: Build context
    dataset, stats = build_context_from_ke_ids(ke_ids)
    ...
    if thread_id != current_id:
        return
    confirm = messagebox.askyesno("Context Check", stat_msg + "\n\nDo you want to continue?")
    if not confirm:
    ...
    result = ""
    final_question = task_text
    if preset_prompt:
        final_question = preset_prompt + "\n\n" + task_text
    try:
...
def import_pdf():
    if 'PdfReader' not in globals():
        messagebox.showerror("Dependency Missing", "pypdf is not installed. Please run: pip install pypdf")
        return
    ...
    try:
        ...
        if not full_text.strip():
            messagebox.showwarning("Warning", "PDF content is empty or could not be extracted.")
            return
        task_entry.delete("1.0", tk.END)
        task_entry.insert("1.0", full_text.strip())
        messagebox.showinfo("Success", f"Imported {len(full_text)} characters from PDF.")
    except Exception as e:
...
def export_html():
    if 'markdown' not in globals():
        messagebox.showerror("Dependency Missing", "markdown is not installed. Please run: pip install markdown")
        return
    global last_result
    if not last_result.strip():
        messagebox.showwarning("Warning", "No output to export. Please run a task first.")
        return

    # Convert Markdown to HTML
    html_body = markdown.markdown(last_result, extensions=['tables', 'fenced_code'])

    # Replace KE+digits with AOP-Wiki links
    html_body = re.sub(r'\bKE(\d+)\b', r'<a href="https://aopwiki.org/events/\1" target="_blank">KE\1</a>', html_body)

    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>AOP-Smart Output Report</title>
    <style>
...
<body>
    {html_body}
    <p style="margin-top: 30px; color: #7f8c8d; font-size: 0.9em;">Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
</body>
</html>"""
...
preset_label_var = tk.StringVar(value="Current Preset: None")
...
# ---------- Preset dropdown ----------
tk.Label(config_frame, text="Preset:", fg="blue").grid(row=8, column=0, sticky="w", pady=(5, 0))
preset_frame = tk.Frame(config_frame)
preset_frame.grid(row=8, column=1, sticky="ew", pady=(5, 0))
...
preset_combo = ttk.Combobox(preset_frame, values=list(presets.keys()), state="readonly", width=25)
...
def on_preset_select(event):
    ...
    if selected in presets:
        preset_prompt = presets[selected]
        preset_label_var.set(f"Current Preset: {selected}")
    else:
        preset_prompt = ""
        preset_label_var.set("Current Preset: None")
...
tk.Button(preset_frame, text="Clear", command=clear_preset).pack(side="left")

preset_status_label = tk.Label(config_frame, textvariable=preset_label_var, fg="green")
preset_status_label.grid(row=9, column=0, columnspan=2, sticky="w", pady=(2, 5))
...
output_box = scrolledtext.ScrolledText(right_frame, state=tk.DISABLED)
output_box.grid(row=1, column=0, sticky="nsew", pady=(0, 10))

# ------- Task area with toolbar -------
task_frame = tk.Frame(right_frame)
task_frame.grid(row=2, column=0, sticky="nsew")
task_frame.grid_columnconfigure(0, weight=1)
task_frame.grid_rowconfigure(2, weight=1)
task_frame.grid_rowconfigure(3, weight=0)

toolbar = tk.Frame(task_frame)
toolbar.grid(row=0, column=0, sticky="ew", pady=(0, 5))
btn_import_pdf = tk.Button(toolbar, text="📄 Import PDF", command=import_pdf)
btn_import_pdf.pack(side="left", padx=(0, 10))
btn_export_html = tk.Button(toolbar, text="💾 Export HTML", command=export_html)
btn_export_html.pack(side="right")

tk.Label(task_frame, text="Task").grid(row=1, column=0, sticky="w")
task_entry = scrolledtext.ScrolledText(task_frame, height=5)
task_entry.grid(row=2, column=0, sticky="nsew", padx=(0, 10), pady=(5, 0))

button_frame = tk.Frame(task_frame)
button_frame.grid(row=2, column=1, sticky="n")
run_button = tk.Button(button_frame, text="Run", command=run_llm_thread, bg="#4CAF50", fg="white", width=10)
run_button.pack(pady=(5, 5))
stop_button = tk.Button(button_frame, text="Stop", command=stop_llm, bg="#f44336", fg="white", width=10, state=tk.DISABLED)
stop_button.pack()
